...
public class SetRoleCommand extends BotSubcommandClass {
    ...
    public void onCommand(SlashCommandInteractionEvent event) {
        ...
        HashMap<String, Object> guildInfo = guildHandler.guildConfigs.get(guild.getId());
        if (guildInfo == null)
            return;
        ...
        if (role == null || !role.containsKey(type)) {
            ...
            reply(event, "Đã thêm " + roleOption.getAsRole().getName() + " vào danh sách " + type, 10);
            guildHandler.save();
        } else {
            ((HashMap<String, String>) guildInfo.get(type)).remove(type);
            reply(event, "Đã xóa " + roleOption.getAsRole().getName() + " khỏi danh sách " + type, 10);
            guildHandler.save();
        }
    }
