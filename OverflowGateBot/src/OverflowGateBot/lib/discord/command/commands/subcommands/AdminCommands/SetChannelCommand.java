...
public class SetChannelCommand extends BotSubcommandClass {
     ...
     * return;
     * HashMap<String, String> channels = (HashMap<String, String>)
     * guildHandler.guildConfigs
     * .get(channel.getGuild().getId()).get(type);
     * if (channels != null && !channels.containsKey(channel.getId())) {
     * ((HashMap<String, String>)
     * guildHandler.guildConfigs.get(channel.getGuild().getId()).get(type))
     * .put(channel.getId(), null);
     * reply(event, "Đã thêm " + channel.getName() + " vào danh sách kênh " + type,
     * 10);
     * } else if (channels != null && channels.containsKey(channel.getId())) {
     * ((HashMap<String, String>)
     * guildHandler.guildConfigs.get(channel.getGuild().getId()).get(type))
     * .remove(channel.getId());
     * reply(event, "Đã xóa " + channel.getName() + " khỏi danh sách kênh " + type,
     * 10);
     * 
     * }
     * }
